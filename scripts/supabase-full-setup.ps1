...
Write-Host "1. Prüfe Supabase CLI..." -ForegroundColor Green
try {
...
Write-Host "   (Öffnet Browser für Authentifizierung)" -ForegroundColor Yellow
supabase login
...
try {
    ...
    Write-Host "   ✗ Fehler beim Verknüpfen: $_" -ForegroundColor Red
    exit 1
}
...
Write-Host ""
Write-Host "5. Führe Datenbank-Migrationen aus..." -ForegroundColor Green
try {
    supabase db push
...
# Initial-Daten einspielen
Write-Host ""
Write-Host "6. Spiele Initial-Daten ein..." -ForegroundColor Green
$setupSql = Get-Content -Path "$PSScriptRoot\setup-database.sql" -Raw
try {
    # Verwende psql oder Supabase SQL Editor
    Write-Host "   ⚠ Bitte führe setup-database.sql manuell im Supabase SQL Editor aus" -ForegroundColor Yellow
    Write-Host "   URL: $SUPABASE_URL/project/sql" -ForegroundColor Cyan
} catch {
    Write-Host "   Hinweis: SQL muss manuell ausgeführt werden" -ForegroundColor Yellow
}

# Edge Function deployen
Write-Host ""
Write-Host "7. Deploye Edge Function 'telegram-bot'..." -ForegroundColor Green
try {
...
Write-Host ""
Write-Host "8. Konfiguriere Telegram Webhook..." -ForegroundColor Green
...
try {
    ...
    if ($botInfo.ok) {
        ...
        $webhookResponse = Invoke-RestMethod -Uri "https://api.telegram.org/bot$TELEGRAM_TOKEN/setWebhook" `
...
Write-Host "1. Öffne Supabase Dashboard:" -ForegroundColor White
Write-Host "   $SUPABASE_URL" -ForegroundColor Cyan
Write-Host ""
Write-Host "2. Führe setup-database.sql im SQL Editor aus" -ForegroundColor White
Write-Host ""
Write-Host "3. Erstelle eine Telegram-Gruppe:" -ForegroundColor White
Write-Host "   - Füge @taxi_center_ost_bot hinzu" -ForegroundColor White
